...
export default function ControlPanel({
  ...
  return (
    <div className="win98-outset p-3 flex flex-col gap-3 w-full lg:w-72 shrink-0">
      {/* Title Bar */}
      ...
      <div className="win98-tabs">
        <button onClick={() => setSection('signal')} className={`win98-tab ${section === 'signal' ? 'active' : ''}`}>
          Generator
        </button>
        <button onClick={() => setSection('filter')} className={`win98-tab ${section === 'filter' ? 'active' : ''}`}>
          Filter
        </button>
        <button onClick={() => setSection('mod')} className={`win98-tab ${section === 'mod' ? 'active' : ''}`}>
          Mod/Math
        </button>
      </div>
      ...
      <div className="win98-outset p-3 flex flex-col gap-3 bg-[#C0C0C0]">
        ...
        {section === 'filter' && (
          <div className="flex flex-col gap-3">
            <div className="win98-inset p-2 flex items-center justify-between">
      ...
      {/* Windows 98 Dialog Buttons */}
      <div className="flex justify-end gap-2 pt-1 border-t border-[#808080]">
        <button className="win98-btn">OK</button>
        <button className="win98-btn">Cancel</button>
        <button className="win98-btn">Apply</button>
      </div>
    </div>
  );
}
