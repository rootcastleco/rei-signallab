...
export default function SignalMetrics({ metrics }) {
  ...
  const cards = [
    ...
    { label: 'THD_PCT', value: metrics.thd_percent || 0, unit: '%', color: '#FF00FF' },
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2">
      {cards.map((c) => (
        <div key={c.label} className="win95-hitcounter flex flex-col justify-between h-14">
          <div className="flex justify-between items-center text-[9px] text-[#808080] font-mono font-bold tracking-wider">
            <span>{c.label}</span>
            <span className="w-1.5 h-1.5 rounded-none bg-[#00FF00]"></span>
          </div>
          <div className="flex items-baseline justify-end gap-1">
            <span className="text-base font-bold font-mono tracking-tight" style={{ color: c.color }}>
              {c.value}
            </span>
