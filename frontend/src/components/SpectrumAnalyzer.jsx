  ...
  return (
    <div className="win98-outset p-2 flex flex-col gap-2">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 py-1 border-b border-[#808080]">
        <div className="flex gap-1">
          <button onClick={() => setLogScale(false)} className={`win98-btn text-xs ${!logScale ? 'bg-[#FFFFFF]' : ''}`}>
            LINEAR
          </button>
          <button onClick={() => setLogScale(true)} className={`win98-btn text-xs ${logScale ? 'bg-[#FFFFFF]' : ''}`}>
            LOG_SCALE
          </button>
        </div>
